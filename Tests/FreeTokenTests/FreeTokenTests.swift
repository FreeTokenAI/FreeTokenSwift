...
//                            throw NSError(domain: "userCancel", code: 100)
    ...
    func testDisableThinking() throws {
        let expectation = self.expectation(description: "Waiting for disable thinking test")

        Task {
            let modelCode = "qwen3_4b"

            await FreeToken.shared.downloadAIModel(modelCode: modelCode) { state in
                // Test with thinking disabled
                let disableThinkingConfig = FreeToken.AIRunConfig(
                    temperature: 0.7,
                    disableThinking: true
                )

                let messages = [FreeToken.Message(role: .user, content: "What is 2+2? Answer with just the number.")]

                do {
                    let responseWithThinkingDisabled = try await FreeToken.shared.localChat(
                        modelCode: modelCode,
                        messages: messages,
                        runIdentifier: "test-disable-thinking",
                        aiRunConfig: disableThinkingConfig
                    )

                    print("✅ Response with thinking disabled: \(responseWithThinkingDisabled.content)")
                    XCTAssertTrue(responseWithThinkingDisabled.content.count > 0, "Expected non-empty response with thinking disabled")
                    XCTAssertTrue(responseWithThinkingDisabled.content.contains("4"), "Expected response to contain '4'")

                    // IMPORTANT: Verify that the response does NOT contain <think> tags when thinking is disabled
                    XCTAssertFalse(responseWithThinkingDisabled.content.contains("<think>"), "Response should contain '<think></think>' when thinking is disabled")
